...
/**
 * Class to get the texture for a specific pathway building and texture type. Implements lazy
 * loading to load textures only when needed.
 */
public class PathwayTextures {
  ...
  /**
   * Map containing the file paths for each texture. This map is initialized with all possible
   * BuildingType and Type combinations.
   */
  private static final HashMap<BuildingType, HashMap<Type, String>> texturePaths =
      new HashMap<>() {
        {
          put(
              BuildingType.ROAD,
  ...
  /** Map containing the loaded textures. Textures are loaded and stored here on demand. */
  private static final HashMap<BuildingType, HashMap<Type, Texture>> textureList = new HashMap<>();

  /**
   * Retrieves the texture for the specified BuildingType and Type. Loads the texture if it hasn't
   * been loaded yet.
   *
    ...
    if (texture == null) {
      ...
      if (path == null) {
        throw new IllegalArgumentException(
            "No texture path defined for BuildingType: "
                + buildingType
                + ", Type: "
                + pathwayTextureType);
      }
  ...
  /**
   * Disposes of all loaded textures. Call this method when textures are no longer needed to free up
   * resources.
   */
