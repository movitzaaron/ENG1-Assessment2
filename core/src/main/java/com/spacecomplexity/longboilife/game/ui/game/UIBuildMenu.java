...
public class UIBuildMenu extends UIElement {
  ...
  public UIBuildMenu(Viewport uiViewport, Table parentTable, Skin skin) {
    ...
    EventHandler.getEventHandler()
        .createEvent(
            EventHandler.Event.CLOSE_BUILD_MENU,
            (params) -> {
              closeMenu();
              return null;
            });
  ...
  /**
   * Open the build menu ready for the user to place buildings from a specific {@link
   * BuildingCategory}.
   *
  ...
  public void openMenu(BuildingCategory category) {
    ...
    for (BuildingType building : buildings) {
      ...
      texture.setMinSize(
          textureSize * (buildingSize.x / maxDimension),
          textureSize * (buildingSize.y / maxDimension));
