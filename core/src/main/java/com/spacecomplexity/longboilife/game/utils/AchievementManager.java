...
public class AchievementManager {
    ...
    public void poll(GameState gameState, Preferences preferences){
        if (!unlockedAchievements.contains(Achievement.BROKE) && gameState.money == 0){
            unlockAchievement(Achievement.BROKE, EventHandler.Event.BROKE_ACHIEVEMENT_DIALOG, preferences);
        }
        else if (!unlockedAchievements.contains(Achievement.SATISFIED) && gameState.satisfactionScore == 1){
            unlockAchievement(Achievement.SATISFIED, EventHandler.Event.SATISFIED_ACHIEVEMENT_DIALOG, preferences);
        }
        else if (!unlockedAchievements.contains(Achievement.FITNESS) && gameState.getBuildingCount(BuildingType.GYM) == 3){
            unlockAchievement(Achievement.FITNESS, EventHandler.Event.FITNESS_ACHIEVEMENT_DIALOG, preferences);
        }
        else if (!unlockedAchievements.contains(Achievement.ACADEMIC) && gameState.getBuildingCount(BuildingType.LIBRARY) == 3){
            unlockAchievement(Achievement.ACADEMIC, EventHandler.Event.ACADEMIC_ACHIEVEMENT_DIALOG, preferences);
        }
        else if (!unlockedAchievements.contains(Achievement.FOODIE) && gameState.getBuildingCount(BuildingType.GREGGS) == 3){
            unlockAchievement(Achievement.FOODIE, EventHandler.Event.FOODIE_ACHIEVEMENT_DIALOG, preferences);
        }
    }
     ...
     * Unlocks the achievement by calling the associated event, and printing achievement name.
     */
    private void unlockAchievement(Achievement achievement, EventHandler.Event event, Preferences preferences) {
