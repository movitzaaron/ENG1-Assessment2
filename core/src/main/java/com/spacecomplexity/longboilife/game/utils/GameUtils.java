...
public class GameUtils {
     ...
     * <p>
     * Satisfaction is generated by multiplying a set of modifiers, all of which are between 0 and 1.
     * distanceModifier is calculated based on the average shortest distance to buildings in other categories
     * from the average accommodation building.
     * buildingCountModifier is calculated based on the building count - fewer buildings means a lower modifier.
     * buildingProportionModifier is calculated based on the difference in number of buildings in each category.
     * emptyCategoryModifier is calculated by working out the proportion of categories that haven't been placed yet.
     *
    ...
    public static void updateSatisfactionScore(World world) {
        ...
        Set<BuildingCategory> searchBuildingCategories = new HashSet<>(Arrays.stream(BuildingCategory.values()).toList());
        ...
        for (Building accommodationBuilding : categorisedBuildings.get(BuildingCategory.ACCOMMODATION)) {
            // each accommodation building gets a distance score based on how close it is to other building categories
            ...
            for (BuildingCategory category : searchBuildingCategories) {
                ...
                // add a value between 0 and 1 to the distance score based on distance from category to accommodation
        ...
        if (searchBuildingCategories.isEmpty() || categorisedBuildings.get(BuildingCategory.ACCOMMODATION).isEmpty()){
            distanceModifier = 0;
        }
        ...
        for (BuildingType buildingType : BuildingType.values()) {
            ...
            if (category != BuildingCategory.PATHWAY) {
                if (buildingCategoryCounts.containsKey(category)) {
                    buildingCategoryCounts.put(category, buildingCategoryCounts.get(category) + gameState.getBuildingCount(buildingType));
                } else {
                    buildingCategoryCounts.put(category, gameState.getBuildingCount(buildingType));
                }
            }
        }
        int greatestCategoryDifference = Collections.max(buildingCategoryCounts.values()) - Collections.min(buildingCategoryCounts.values());
