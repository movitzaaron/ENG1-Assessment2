...
public class InputManager {
  ...
  /**
   * Handles continuous input which won't be processed in {@link InputManager.InputProcessor} (as
   * this is event driven). Should be called every frame (before rendering).
   */
  public void handleContinuousInput() {
    ...
    if (!GameState.getState().isTyping) {
      ...
      float cameraSpeed =
          gameState.cameraSpeed * deltaTime * MainCamera.camera().zoom * gameState.scaleFactor;
  ...
  /**
   * Handles input (event driven). Will be called every frame (before rendering, hence before {@link
  ...
  private class InputProcessor extends InputAdapter {
    ...
    public boolean scrolled(float amountX, float amountY) {
      ...
      MainCamera.camera()
          .zoomAt(
              amountY * gameState.cameraScrollZoomSpeed * deltaTime * MainCamera.camera().zoom,
              mousePosition);
    ...
    public boolean keyDown(int keycode) {
      ...
      if (keycode == Keybindings.CANCEL.getKey()) {
        ...
        eventHandler.callEvent(
            GameState.getState().paused
                ? EventHandler.Event.RESUME_GAME
                : EventHandler.Event.PAUSE_GAME);
