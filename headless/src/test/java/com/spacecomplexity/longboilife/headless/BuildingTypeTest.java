...
/** Unit tests for the BuildingType enum. TEST REF : 3 */
...
public class BuildingTypeTest extends AbstractHeadlessGdxTest {
  ...
  public void testBuildingTypeProperties() {
    ...
    assertEquals(
        "buildings/library.png", library.getTexturePath(), "LIBRARY texture path mismatch");
    assertEquals(new Vector2Int(4, 4), library.getSize(), "LIBRARY size mismatch");
    assertEquals(
        BuildingCategory.EDUCATIONAL,
        library.getCategory(),
        "LIBRARY category should be EDUCATIONAL");
    ...
    assertEquals(
        BuildingCategory.RECREATIONAL, gym.getCategory(), "GYM category should be RECREATIONAL");
    ...
    assertEquals(
        ...
        halls.getCategory(),
        "HALLS category should be ACCOMMODATION");
    ...
    assertEquals(
        "buildings/roads/straight.png", road.getTexturePath(), "ROAD texture path mismatch");
  ...
  public void testGetBuildingsOfType() {
    ...
    BuildingType[] accommodationBuildings =
        BuildingType.getBuildingsOfType(BuildingCategory.ACCOMMODATION);
    assertEquals(1, accommodationBuildings.length, "There should be 1 ACCOMMODATION building type");
    assertEquals(
        BuildingType.HALLS, accommodationBuildings[0], "ACCOMMODATION building should be HALLS");

    // Test for EDUCATIONAL
    BuildingType[] educationalBuildings =
        BuildingType.getBuildingsOfType(BuildingCategory.EDUCATIONAL);
    assertEquals(1, educationalBuildings.length, "There should be 1 EDUCATIONAL building type");
    assertEquals(
        BuildingType.LIBRARY, educationalBuildings[0], "EDUCATIONAL building should be LIBRARY");
  ...
  /**
   * Helper method to access the private 'texture' field of BuildingType enum via reflection. This
   * is necessary to verify that the texture has been set to null after disposal.
   *
