...
/** Unit tests for the GameState class. TEST REF : 6 */
...
class GameStateTest extends AbstractHeadlessGdxTest {
  ...
  void testDefaultValuesAfterReset() {
    ...
    assertEquals(
        32f, gameState.cameraScrollZoomSpeed, "Default cameraScrollZoomSpeed should be 32");
    ...
    assertEquals(
        0f, gameState.satisfactionScoreVelocity, "Default satisfactionScoreVelocity should be 0");
    assertFalse(
        gameState.satisfactionModifierPositive,
        "Default satisfactionModifierPositive should be false");
  ...
  void testChangeBuildingCount() {
    ...
    assertEquals(
        5, gameState.getBuildingCount(buildingType), "Building count should be 5 after adding 5");

    gameState.changeBuildingCount(buildingType, -2);
    assertEquals(
        3, gameState.getBuildingCount(buildingType), "Building count should be 3 after removing 2");

    gameState.changeBuildingCount(buildingType, 0);
    assertEquals(
        3,
        gameState.getBuildingCount(buildingType),
        "Building count should remain 3 after no change");
  }
  ...
  void testBuildingsCountIsolation() {
    ...
    assertEquals(
        4,
        gameState.getBuildingCount(buildingType),
        "Original building count should remain unchanged");
    assertEquals(
        10,
        gameState.getBuildingCount(BuildingType.HALLS),
        "Newly added building count should be reflected");
  }
  ...
  void testResetBuildingsCount() {
    ...
    gameState.changeBuildingCount(buildingType, 5);
    assertEquals(
        5, gameState.getBuildingCount(buildingType), "Building count should be 5 before reset");
  ...
  void testModifyBuildingsCount_Reflection() throws NoSuchFieldException, IllegalAccessException {
    ...
    HashMap<BuildingType, Integer> buildingsCount =
        (HashMap<BuildingType, Integer>) buildingsCountField.get(gameState);
    ...
    assertEquals(
        15,
        gameState.getBuildingCount(BuildingType.GREGGS),
        "Building count should reflect changes made via reflection");
  }
}
